...
\begin{center}
    ...
    {\normalsize Duc, Giang, Tri, Linh}\\[2pt]
    ...
    \quad\textbar\quad % TODO: confirm/replace GitHub URL
    Source: \href{https://github.com/}{GitHub repository}}
\end{center}
...
\textbf{User goal:} understand the current citywide situation in seconds and know what it means.
\textbf{Encodings \& rationale:} a large hero AQI number with a six-band scale marker gives an immediate
...
\textbf{Interaction:} a 3D$\leftrightarrow$2D toggle (the 2D scatter-mapbox is the accessible fallback),
map hover tooltips, and optional realtime polling that refreshes the snapshot and re-runs the 1~h forecast
(Figure~\ref{fig:overview}).
\textbf{Key insight:} a live reading is immediately legible \emph{and} actionable.
\textbf{Narrative link:} establishes the ``now'' anchor before the user explores space and time.

\begin{figure}[htbp]
\centering
\begin{subfigure}{0.86\textwidth}\centering
...
\textbf{User goal:} find which districts are most polluted and how the ranking shifts by month/year.
\textbf{Encodings \& rationale:} a choropleth is the canonical value-per-region encoding; selected
...
\textbf{Interaction (the strongest coordinated-views story):} one district selection --- made by checkbox
...
\textbf{Key insight:} citywide averages hide real spatial disparities, and linked selection lets users
attribute high months to specific districts.
\textbf{Narrative link:} the \emph{space} axis of the story.
...
\textbf{User goal:} see daily, seasonal, and multi-year patterns and inspect unusual events.
\textbf{Encodings \& rationale:} a GitHub-style \emph{calendar heatmap} (day-of-week $\times$ week, AQI
...
\textbf{Interaction:} pollutant (AQI/PM2.5/PM10), year, and month selectors re-target the views, and a
...
\textbf{Key insight:} Hanoi air quality has strong temporal structure; clean visuals and on-demand
anomaly inspection can coexist.
\textbf{Narrative link:} the \emph{time} axis of the story.
...
\textbf{User goal:} see short-term predicted risk and judge whether to trust it.
\textbf{Encodings \& rationale:} the forecast hero reuses the AQI scale and adds a \emph{source-tier chip}
...
\textbf{Interaction:} horizon (1/6/24~h) and target (AQI/PM2.5) radios reload the relevant cached model;
a PCA colour-mode toggle re-encodes without recomputation.
\textbf{Key insight:} a forecast is only useful if its drivers and error are visible --- so the page sells
interpretability, not just a value.
\textbf{Narrative link:} the \emph{future} axis, closing the space$\rightarrow$time$\rightarrow$future arc.
